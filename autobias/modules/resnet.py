"""
Modified from torchvision/models/resnet.py

The main change is to return the output of all intermediate layers, so they can be used for the
lower capacity model, and to follow the `Layer` interface
"""
...
from autobias.config import TORCHVISION_CACHE_DIR
...
class ResNet(Layer):
    ...
    def __init__(self, arch, block, layers, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, pretrained_url=None, from_pretrained=False,
                 classes=None, n_layers=None):
    ...
    def reset_parameters(self):
      if not self.from_pretrained:
        ...
        state_dict = load_state_dict_from_url(model_urls[self.arch], TORCHVISION_CACHE_DIR, progress=True)
        if self.fc is None:
...
def _resnet(arch, block, layers, **kwargs):
    return ResNet(arch, block, layers, **kwargs)
